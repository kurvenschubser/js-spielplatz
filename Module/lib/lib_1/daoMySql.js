//Programmierhilfe Stammdaten DAO MYSQL
let dao=(function(){
	"use strict";
	let sprDao=(function(){
		var lst=[];
			...
			return new Promise((resolve, reject) => {
				if(lst===null || lst.length ==0){
					...
					try{
							...
							function(response) {
								response.text().then(function(text) {
									...
									try{
										for (var key in doc) {
											var obj = doc[key];
											let ge = new dom.f_sprache(obj.id,obj.bez,obj.beschr,obj.datum,obj.edit);
											lst.push(ge);
										}
										resolve(lst);
									}
						...
						.catch(e=>{reject(e);})//ende fetch.catch
					}
					catch (e) {	reject(e);}
				}
				else {resolve(lst)}
	...
	let temDao=(function(){
							...
							function(response) {
								response.text().then(function(text) {
									...
									try{
										...
										for (var key in doc) {
											var obj = doc[key];
											//id,name,sprache,date,edit
											let ge = new dom.f_thema(obj.id,obj.titel,obj.spr,obj.dateum,obj.edit);
											lst.push(ge);
										}
									}
									catch (e) {reject(e)}
		...
		return {getList,getById,getList,getLstByArt};
	})();
								...
								response.text().then(function(text) {
									...
									try{
										...
										if(doc.art && doc.art == 'Error'){
											reject(Error(doc.msg));
										}
										for (var key in doc) {
											var obj = doc[key];
											let ge = new dom.f_eintrag(obj.id,obj.titel,'-',obj.text,obj.sub,obj.lang,obj.datum,obj.edit);
											lst.push(ge);
										}
									}
									catch (e) { throw e }
